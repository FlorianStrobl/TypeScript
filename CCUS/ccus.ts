...
const preservedCharacters: string[] = [
...
function RunCC(mainFile: string, otherFiles?: string[], headers?: string[]) {
  mainFile = preCompile(
    mainFile,
    [
      { name: 'file1', content: '-' },
  ...
  const mainFunctions = getTopLevelFunctions(mainFile);
...
const isWord = (word: string) => new RegExp(`\b${word}\b`, 'g');

function callFunction(func: string, ctx: IFunctionContext): string {
...
function preCompile(
  ...
  } = extractSubstrings(file, '$', subStringRegex);
  ...
  defs = defs.map((def) => ({
    ...
    value: insertSubstrings(def.value, /\$\d+/g, substrs)
  }));
  headers = headers.map((header) =>
    insertSubstrings(header, /\$\d+/g, substrs)
  );

  // TODO def and header/use/inc

  // reinsert all the subtrings, which where removed at the beginning
  file = insertSubstrings(lines.join(''), /\$\d+/g, substrs);
...
// TODO, not use
function getTopLevelFunctions(str: string): IFunction[] {
  ...
  for (let i = 0; i < chars.length; ++i) {
    ...
    if (isFunc) {
      ...
      if (funcNameSave) {
        if (chars[i] === ' ' && isValidType(curFuncName)) {
  ...
  for (const func of finishedFuncs)
    if (isValidFunc(func) !== true) throw isValidFunc(func);
...
function getLogicalCodeBlocks(code: string): string[] {
  let blocks: string[] = [];
...
function isValidFunc(func: IFunction): Error | boolean {
  if (!isValidFuncName(func.name))
...
function isValidType(str: Type | string): boolean {
  for (const type in Type) if (str === type) return true;
  return false;
}

function isValidVarName(str: string): boolean {
  return (
    isAlphanumeric(str) &&
    isLetter(str.split('')[0]) &&
    str !== '' &&
    str !== ' '
  );
}

function isValidFuncName(str: string): boolean {
  return (
    isAlphanumeric(str) &&
    isLetter(str.split('')[0]) &&
    str !== '' &&
...
function isValidClassName(str: string): boolean {
  return (
    isAlphanumeric(str) &&
    isLetter(str.split('')[0]) &&
    str !== '' &&
