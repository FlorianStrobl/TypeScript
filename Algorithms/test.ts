...
namespace motors {
  ...
  export function init() {
    ...
    motorMM = control.mmap(
      '/dev/lms_motor',
      MotorDataOff.Size * DAL.NUM_OUTPUTS,
      0
    );
  ...
  export class MotorBase extends control.Component {
    ...
    run(
      speed: number,
      value: number = 0,
      unit: MoveUnit = MoveUnit.MilliSeconds
    ) {
      this.init();
      ...
      steps[0] =
        (useSteps ? this._accelerationSteps : this._accelerationTime) || 0;
      steps[2] =
        (useSteps ? this._decelerationSteps : this._decelerationTime) || 0;
    ...
    setRunPhase(
      ...
      switch (unit) {
        case MoveUnit.Rotations:
          ...
          if (phase == MovePhase.Acceleration) this._accelerationSteps = temp;
          else this._decelerationSteps = temp;
          break;
        case MoveUnit.Degrees:
        ...
        case MoveUnit.Seconds:
          ...
          if (phase == MovePhase.Acceleration) this._accelerationTime = temp;
          else this._decelerationTime = temp;
          break;
    ...
    private _run(speed: number) {
      ...
      if (this._accelerationTime) {
        this._schedule({
          speed: speed,
          useSteps: false,
          steps: [this._accelerationTime, 100, 0]
        });
      ...
      const b = mkCmd(
        this._port,
        this._regulated ? DAL.opOutputSpeed : DAL.opOutputPower,
        1
      );
      ...
      for (let i = 0; i < DAL.NUM_OUTPUTS; ++i) {
        if (this._port & (1 << i)) {
          motorMM.setNumber(
            ...
            i * MotorDataOff.Size + MotorDataOff.TachoSensor,
            0
          );
        }
      }
  ...
  export class SynchedMotorPair extends MotorBase {
    ...
    tank(
      ...
      const speed =
        Math.abs(speedLeft) > Math.abs(speedRight) ? speedLeft : speedRight;
      let turnRatio =
        speedLeft == speed
          ? speedLeft == 0
            ? 0
  ...
  function outOffset(out: Output) {
    for (let i = 0; i < DAL.NUM_OUTPUTS; ++i) {
      if (out & (1 << i)) return i * MotorDataOff.Size;
    }
    return 0;
  }
  ...
  export function getAllMotorData(): MotorData[] {
    init();
    return [Output.A, Output.B, Output.C, Output.D].map((out) =>
      getMotorData(out)
    );
  }
