...
namespace AStar {
    ...
    function cheapestTraversedField(): Vector2d {
        ...
        for (let y = 0; y < fieldYLength; ++y)
            for (let x = 0; x < fieldXLength; ++x) {
                const cheapestFieldCost: number = fields[cheapestField.y][cheapestField.x].gCost + fields[cheapestField.y][cheapestField.x].hCost;
                const currentFieldCost: number = fields[y][x].gCost + fields[y][x].hCost;
                // if the field was explored and not traversed or nothing
                // and it is cheaper, update it TODO
                if (fields[y][x].explored === 1 && currentFieldCost <= cheapestFieldCost)
                    cheapestField = { x: x, y: y };
            }
    ...
    function expFlds(coords: Vector2d): boolean {
        ...
        fields[coords.y][coords.x].explored = 2; // update current field state
        // value of current field
